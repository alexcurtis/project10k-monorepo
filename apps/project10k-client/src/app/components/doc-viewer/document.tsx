...
export function CompanyDocument() {
    ...
    useEffect(() => {
        ...
        const loadCitationsCb = () => {
            loadCitations(iframeRef, citations);
        };

        if (data && html) {
            const iframe = iframeRef.current;
            iframe?.addEventListener("load", loadCitationsCb);

            return () => {
                iframe?.removeEventListener("load", loadCitationsCb);
            };
        }
    }, [data, html, workspace, iframeRef]);
...
//     const citations = workspace.journals.flatMap((journal) => {
//         return journal.citations.flatMap((citation) => {
//             return citation.filing._id === filing._id ? citation : [];
//         });
//     });
